...
def conv(image, filters, padding=0, stride=1):
    ...
    output_channels, input_channels_f, H_f, W_f = filters.shape
    if isinstance(stride,int):
        stride= (stride,stride)
    assert (height - H_f) % stride[0] == 0, '步长必须能够被整除'
    assert (width - W_f) % stride[1] == 0, '步长必须能够被整除'
    conv_z = np.zeros((batch_size, output_channels, 1 + (height - H_f) // stride[0], 1 + (width - W_f) // stride[1]),dtype = np.int32)
        ...
        for d in np.arange(output_channels):
            for h in np.arange(height - H_f + 1)[::stride[0]]:
                for w in np.arange(width - W_f + 1)[::stride[1]]:
                    conv_z[n, d, h // stride[0], w // stride[1]] = np.sum(padding_image[n, :, h:h + H_f, w:w + W_f] * filters[d,:] % encodeFP32.module) % encodeFP32.module
